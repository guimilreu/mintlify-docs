...
| `batch` | object | **Opcional.** Só aparece quando o evento veio de um processamento em massa. `origin` diz a origem (`import`) e `detail` o arquivo, quando houver. |

<Note>
  **Rajada.** Uma importação de planilha pode gerar centenas de eventos de uma vez — por exemplo,
  300 `contact.lifecycle_changed` de uma lista de prospecção. Esses eventos chegam com `batch` no
  envelope, e saem numa fila de cadência própria: o backlog da importação não atrasa os eventos
  avulsos da mesma organização. Use `batch` para agrupar em vez de tratar cada um como uma mudança
  individual do cliente.

  ```json
  "batch": { "origin": "import", "detail": "clinicas.csv" }
  ```
</Note>
...
| `contact.lifecycle_changed` | Ciclo de vida mudou |
...
| `proposal.action` | Ação clicada na proposta |
...
| `atendimento.opened` | Atendimento aberto |
| `atendimento.status_changed` | Status do atendimento mudou |
| `atendimento.assigned` | Atendente definido |
| `atendimento.closed` | Atendimento encerrado |
| `atendimento.reopened` | Atendimento reaberto |
| `atendimento.discarded` | Atendimento descartado |
| `atendimento.csat_received` | Avaliação recebida |
| `atendimento.sla_breached` | SLA estourado |
...
| `call.started` | Ligação atendida |
| `call.ended` | Ligação encerrada |
| `call.missed` | Ligação perdida |
| `call.handoff_requested` | Ligação passada para humano |
| `call.recording_ready` | Gravação disponível |
| `broadcast.completed` | Disparo concluído |
| `broadcast.replied` | Contato respondeu ao disparo |
...
| `team.member.deactivated` | Membro desativado |
| `team.member.reactivated` | Membro reativado |
...
O contato mudou de estágio — `unknown`, `lead`, `opportunity`, `customer` ou `former_customer`. Derivado de leads e negócios, nunca digitado; `reason` diz o evento que causou.
...
Um negócio ganho ou perdido voltou a ficar aberto. `previous_status` diz de onde ele veio.
...
```json
{
  ...
  "data": {
    "proposal": {
      ...
      "public_url": "https://socialsell.ai/p/9f2b7c1e-4a83-4d51-9e0b-2c6f1a8d3b47",
      "view_count": 3,
...
A proposta foi publicada e o link do cliente passou a valer.
...
O cliente clicou em um botão da proposta. Só dispara para botões com automação ligada no editor.
...
Uma conversa recebida virou atendimento (ou alguém abriu um à mão / por automação).

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "atendimento.opened",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "atendimento": null,
...
### `atendimento.status_changed`

Aberto ↔ aguardando cliente ↔ aguardando interno. `from` e `to` dizem a transição.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "atendimento.status_changed",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "atendimento": null,
...
### `atendimento.assigned`

O atendimento ganhou (ou trocou de) atendente — acompanha a conversa enquanto aberto.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "atendimento.assigned",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "atendimento": null,
...
Encerrado com `category` e `close_reason`. A conversa vai para Concluídas.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "atendimento.closed",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "atendimento": null,
...
### `atendimento.reopened`

O cliente voltou (ou alguém reabriu): nasce um NOVO atendimento, `reopened_from` aponta o anterior.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "atendimento.reopened",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "atendimento": null,
...
### `atendimento.discarded`

Aberto por engano e apagado — a conversa virou modo Vendas. NÃO conta como atendimento encerrado: não entra em SLA nem em "Por que nos procuram".

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "atendimento.discarded",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "atendimento": null,
...
### `atendimento.csat_received`

O cliente respondeu a pesquisa de satisfação. `score` de 1 a 5.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "atendimento.csat_received",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "atendimento": null,
...
### `atendimento.sla_breached`

Prazo de 1ª resposta ou de resolução vencido. `kind` diz qual. Sai uma vez por prazo.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "atendimento.sla_breached",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "atendimento": null,
...
<Info>
  Toda ligação carrega `channel`, que diz por onde ela passou: `whatsapp` (chamada de voz do WhatsApp) ou `phone` (telefone comum). Em `phone`, `line_id` traz a linha telefônica que atendeu ou discou; no WhatsApp ele é `null`. Os dois campos foram acrescentados em 02/set/2026 e nunca faltam: integração escrita antes disso continua funcionando sem tocar em nada.
</Info>

### `call.started`

Uma ligação começou a ser atendida — por uma pessoa ou por um agente de IA. `answered_by` diz qual dos dois.
...
```json
{
  ...
  "data": {
    "call": {
      ...
      "from": "+5511999998888",
      "to": "+551133334444",
      "answered_by_kind": "agent",
      "conversation_id": "664d1e2f3a4b5c6789012348",
      "contact_id": "664f1a2b3c4d5e6f78901234",
      "duration_sec": 0,
      "started_at": "2026-08-20T14:26:00.000Z",
      "answered_at": "2026-08-20T14:26:12.000Z",
      "ended_at": null,
      "disposition": null
    },
    "contact": {
...
A ligação terminou. `reason` é um código estável para ramificar; `summary` só vem quando quem atendeu foi um agente de IA.
...
```json
{
  ...
  "data": {
    "call": {
      ...
      "started_at": "2026-08-20T14:26:00.000Z",
      "answered_at": "2026-08-20T14:26:12.000Z",
      "ended_at": "2026-08-20T14:30:00.000Z",
      "disposition": {
        "code": "resolvido",
        "label": "Resolvido na ligação"
      }
    },
    "contact": {
      "id": "664f1a2b3c4d5e6f78901234",
    ...
    "duration_sec": 214,
    "ai_duration_sec": 198,
    "summary": "Cliente perguntou o prazo de implantação e pediu para revisar a cláusula de reajuste. Ficou de responder até sexta."
  }
}
```
...
Ninguém atendeu — nem pessoa, nem agente de IA. É o gancho para acordar um fluxo de retorno no sistema do integrador.
...
```json
{
  ...
  "data": {
    ...
    "rang_for": [
      {
        "user_id": "664a0000000000000000000f",
        "result": "timeout"
      },
      {
        "user_id": "664a00000000000000000010",
        "result": "rejected"
      }
    ]
  }
...
O agente de IA passou a ligação para uma pessoa. `reason` é a frase do próprio agente (o caso comum é o cliente pedir) e `ringing` diz se chegou a tocar para alguém.
...
A gravação da ligação terminou de ser processada e já pode ser ouvida no painel. O áudio não viaja no webhook.
...
Um contato respondeu a um disparo em massa (primeira resposta, até 72 h depois do envio). Uma vez por contato por disparo.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "broadcast.replied",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "broadcast": {
...
Alguém perdeu o acesso à organização. O histórico permanece, e o payload diz para quem foi o trabalho.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "team.member.deactivated",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "member": {
...
Alguém voltou a ter acesso à organização. Reativar devolve acesso e cargo, nunca a carteira.

```json
{
  "id": "evt_9f2c1a7b4e8d0c3a5b6f7e81",
  "type": "team.member.reactivated",
  "created_at": "2026-08-20T14:30:00.000Z",
  "organization_id": "664a1b2c3d4e5f6789012345",
  "data": {
    "member": {
