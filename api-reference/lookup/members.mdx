...
| `is_active` | boolean | ⚠️ Sempre `true` — não reflete estado real |
| `last_active_at` | string\|null | Última vez ativo no painel |
| `avatar_url` | string\|null | URL do avatar |

<Note>
  `is_active` devolve `true` para todo membro listado; ele não distingue quem está inativo. Use
  `last_active_at` para inferir atividade recente.
</Note>
