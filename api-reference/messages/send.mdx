...
<Warning>
  `template_name` só é considerado quando o número é do **WhatsApp Oficial**. Em um número por QR Code
  ele é **ignorado em silêncio** e a API envia o `content` como texto comum — que no exemplo abaixo está
  vazio, resultando em mensagem em branco.

  Confirme o canal do número antes: o campo `channel` (`official` \| `qrcode`) aparece em
  [`whatsapp_instances` de um disparo](/api-reference/broadcasts/list).
</Warning>
