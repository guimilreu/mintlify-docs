---
title: "Criar campo personalizado"
description: "Cria um novo campo personalizado para contatos, empresas ou negócios."
---

## Endpoint

```
POST /v1/custom-fields
```

**Escopo necessário:** `write:custom_fields`

## Corpo da requisição

| Campo | Tipo | Obrigatório | Descrição |
|-------|------|-------------|-----------|
| `key` | string | Sim | Chave única. Apenas minúsculas, dígitos e `_`, iniciando com letra. Máx. 50 |
| `label` | string | Sim | Rótulo de exibição. Máx. 100 |
| `type` | string | Sim | Tipo do campo (ver tabela abaixo) |
| `entity` | string | Sim | `contact`, `company` ou `deal` |
| `options` | array | Não | Opções para `select`/`multiselect`. Strings ou `{value, label}` |
| `default_value` | any | Não | Valor padrão |
| `required` | boolean | Não | Se é obrigatório |

## Tipos de campo disponíveis

| Tipo | Descrição |
|------|-----------|
| `text` | Texto curto |
| `textarea` | Texto longo |
| `number` | Número |
| `currency` | Valor monetário |
| `date` | Data |
| `datetime` | Data e hora |
| `boolean` | Verdadeiro/falso |
| `select` | Seleção única |
| `multiselect` | Seleção múltipla |
| `url` | URL |
| `email` | E-mail |
| `phone` | Telefone |

## Exemplo de requisição

```bash
curl -X POST https://api.socialsell.ai/v1/custom-fields \
  -H "Authorization: Bearer sk_live_..." \
  -H "Content-Type: application/json" \
  -d '{
    "key": "origem_lead",
    "label": "Origem do Lead",
    "type": "select",
    "entity": "contact",
    "options": ["Site", "Indicação", "Evento", "Anúncio"]
  }'
```

## Resposta

Retorna `HTTP 201` com o campo criado.

<Warning>
`key`, `type` e `entity` não podem ser alterados após a criação. Defina-os com cuidado.
</Warning>

## Regras da `key`

A `key` é o identificador do campo dentro do objeto `custom_fields` — e é **imutável** depois de criada.

| Regra | Valor |
|-------|-------|
| Formato | `^[a-z][a-z0-9_]*$` — começa com letra minúscula; depois, minúsculas, dígitos e `_` |
| Tamanho | Até 50 caracteres |
| Unicidade | Por entidade. `cpf` em `contact` e `cpf` em `company` convivem |

✅ `cpf`, `data_nascimento`, `nivel_2`  ·  ❌ `CPF`, `2_via`, `data-nascimento`, `data nascimento`

## Erros

| Código | Status | Descrição |
|--------|--------|-----------|
| `VALIDATION_ERROR` | `400` | Corpo inválido — inclusive `key` fora do formato |
| `DUPLICATE_KEY` | `409` | Já existe campo com essa `key` nesta entidade |
