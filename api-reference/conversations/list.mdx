...
| `channel` | string | `whatsapp`, `instagram`, `messenger`, `web_chat` (chat do site) ou `other` |
...
| `contact` | object \| null | Contato `{id, name, phone}` |
| `channel` | string | `whatsapp`, `instagram`, `messenger`, `web_chat` ou `other` |
| `channel` | string | `whatsapp`, `instagram`, `messenger`, `web_chat` (chat do site) ou `other` |
...
| `ai_agent` | object\|null | Agente ativo `{id, name, paused, activated_at}`. Ver nota sobre `paused` |
...
<Note>
  A listagem devolve apenas conversas de atendimento (`type: "direct"`). O chat interno da equipe nunca
  aparece aqui.
</Note>

<Note>
  `ai_agent.paused` é legado: ele **não** reflete uma pausa daquela conversa, e sim se o atendimento
  automático da organização está desligado. O nome foi mantido para não quebrar integrações existentes.
</Note>
